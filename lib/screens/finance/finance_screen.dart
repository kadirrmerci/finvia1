...
class _FinanceScreenState extends State<FinanceScreen>
  ...
  double get _totalDebt =>
      _debts.fold(0, (sum, d) => sum + d.remainingAmount);
  ...
  Widget _buildOverview() {
    ...
    final thisMonthTx = _transactions.where((t) =>
        t.isExpense &&
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Column(children: [
            ...
            Text('₺${NumberFormat('#,##0.00').format(_balance)}',
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
              _balanceChip(Icons.arrow_upward, 'Gelir', _totalIncome, Colors.greenAccent),
              _balanceChip(Icons.arrow_downward, 'Gider', _totalExpense, Colors.redAccent),
            ]),
          ]),
        ),
        const SizedBox(height: 16),
        GridView.count(
          ...
          children: [
            _infoCard('Tasarruf Oranı', '%${savingsRate.toStringAsFixed(1)}', Icons.savings, Colors.green),
            _infoCard('En Çok Harcama', topCategory?.key ?? '-', Icons.category, Colors.orange),
            _infoCard('Günlük Ort.', '₺${NumberFormat('#,##0').format(dailyAvg)}', Icons.today, Colors.blue),
            _infoCard('Abonelikler', '₺${NumberFormat('#,##0.00').format(_totalSubscriptions)}/ay', Icons.subscriptions, Colors.purple),
          ],
        ),
        const SizedBox(height: 16),
        if (_creditCards.isNotEmpty)
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              Text('₺${NumberFormat('#,##0.00').format(_totalCreditCardDebt)}',
        ...
        if (_categoryTotals.isNotEmpty) ...[
          ...
          Wrap(
            spacing: 8,
            runSpacing: 4,
            children: _categoryTotals.entries.toList().asMap().entries.map((e) {
        ...
        if (_monthlyExpenses.length > 1) ...[
          const Text('Aylık Harcama Trendi',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
        ...
        if (_debts.isNotEmpty)
          Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              Text('₺${NumberFormat('#,##0.00').format(_totalDebt)}',
  ...
  List<PieChartSectionData> _buildPieSections() {
    ...
    return entries.asMap().entries.map((e) {
      final percent = _totalExpense > 0 ? e.value.value / _totalExpense * 100 : 0.0;
    ...
    if (_transactions.isEmpty) {
      return const Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          SizedBox(height: 16),
          Text('Henüz işlem yok', style: TextStyle(color: Colors.grey, fontSize: 16)),
          Text('+ butonuna basarak ekle', style: TextStyle(color: Colors.grey)),
        ]),
      );
    }
          ...
          child: Card(
            ...
            child: ListTile(
              ...
              subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('${t.category} • ${DateFormat('dd MMM yyyy').format(t.date)}${t.isFixed ? ' • Zorunlu' : ''}',
                    style: const TextStyle(fontSize: 12)),
                if (t.creditCardName != null)
                  Text('💳 ${t.creditCardName}',
  ...
  Widget _buildSubscriptions() {
    return Column(children: [
      Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Aylık Toplam', style: TextStyle(color: Colors.white70)),
          Text('₺${NumberFormat('#,##0.00').format(_totalSubscriptions)}/ay',
      ...
      Expanded(
        child: _subscriptions.isEmpty
            ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                SizedBox(height: 16),
                Text('Abonelik eklenmedi', style: TextStyle(color: Colors.grey, fontSize: 16)),
              ]))
            : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Dismissible(
                    ...
                    onDismissed: (_) async {
                      await NotificationService().cancelNotification(2000 + s.id.hashCode);
                    ...
                    child: Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                          ...
                          child: Text(s.title[0].toUpperCase(),
                              style: const TextStyle(color: Color(0xFF6C63FF), fontWeight: FontWeight.bold)),
                        ),
                        title: Text(s.title, style: const TextStyle(fontWeight: FontWeight.w600)),
                        subtitle: Text('Her ayın ${s.billingDay}. günü • $daysLeft gün kaldı'),
                        trailing: Text('₺${NumberFormat('#,##0.00').format(s.amount)}',
      ...
      Expanded(
        child: _debts.isEmpty
            ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                SizedBox(height: 16),
                Text('Borç eklenmedi', style: TextStyle(color: Colors.grey, fontSize: 16)),
              ]))
            : ListView.builder(
                padding: const EdgeInsets.symmetric(horizontal: 16),
                      ...
                      child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            ...
                            Text('₺${NumberFormat('#,##0').format(d.remainingAmount)} kaldı',
                          ...
                          const SizedBox(height: 8),
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            Text('${(progress * 100).toStringAsFixed(0)}% ödendi',
                                style: const TextStyle(fontSize: 12, color: Colors.grey)),
                            Text('~${d.remainingMonths} ay kaldı',
                                style: const TextStyle(fontSize: 12, color: Colors.grey)),
                          ]),
                          const SizedBox(height: 4),
                          Text('Tahmini bitiş: ${DateFormat('MMM yyyy').format(d.estimatedEndDate)}',
                              style: const TextStyle(fontSize: 12, color: Colors.grey)),
                          const SizedBox(height: 8),
                          ElevatedButton.icon(
                            ...
                            label: Text('Ödeme Yap (₺${NumberFormat('#,##0').format(d.monthlyPayment)})'),
                          ),
                        ]),
                      ),
                    ),
  ...
  void _showPayDebt(Debt d) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('₺${NumberFormat('#,##0.00').format(d.monthlyPayment)} ödeme yapılsın mı?'),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              final updated = Debt(
                id: d.id, title: d.title, totalAmount: d.totalAmount,
                paidAmount: d.paidAmount + d.monthlyPayment,
                monthlyPayment: d.monthlyPayment, startDate: d.startDate,
                interestRate: d.interestRate,
              );
              await _db.updateDebt(updated);
              final t = FinanceTransaction(
                id: _uuid.v4(), title: '${d.title} Borç Ödemesi',
                amount: d.monthlyPayment, category: 'Borç Ödemesi',
                date: DateTime.now(), isExpense: true, isFixed: true,
              );
  ...
  Widget _buildBudget() {
    ...
    final monthBudgets = _budgets.where((b) => b.month == currentMonth).toList();
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (monthBudgets.isEmpty)
          const Center(
            child: Padding(
              ...
              child: Column(children: [
                ...
                SizedBox(height: 16),
                Text('Bütçe hedefi eklenmedi', style: TextStyle(color: Colors.grey, fontSize: 16)),
              ]),
            ),
          )
        else
          ...monthBudgets.map((b) {
            ...
            final progress = b.limitAmount > 0 ? (spent / b.limitAmount).clamp(0.0, 1.0) : 0.0;
            ...
            return Card(
              ...
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    ...
                    if (isOver)
                      Container(
                        ...
                        child: Text('⚠️ ₺${(spent - b.limitAmount).toStringAsFixed(2)} aşıldı!',
                  ...
                  const SizedBox(height: 8),
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    Text('₺${NumberFormat('#,##0').format(spent)} harcandı',
                        style: TextStyle(color: isOver ? Colors.red : Colors.grey, fontSize: 12)),
                    Text('Limit: ₺${NumberFormat('#,##0').format(b.limitAmount)}',
  ...
  Widget _buildCreditCards() {
    if (_creditCards.isEmpty) {
      return const Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.credit_card, size: 64, color: Colors.grey),
          SizedBox(height: 16),
          Text('Henüz kart eklenmedi', style: TextStyle(color: Colors.grey, fontSize: 16)),
          Text('+ butonuna basarak ekle', style: TextStyle(color: Colors.grey)),
        ]),
      );
    }
    return Column(children: [
      Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Toplam Kart Borcu', style: TextStyle(color: Colors.white70)),
          Text('₺${NumberFormat('#,##0.00').format(_totalCreditCardDebt)}',
              style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
        ]),
      ),
                ...
                child: Column(children: [
                  Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text('Güncel Borç', style: TextStyle(color: Colors.white70, fontSize: 11)),
                          Text('₺${NumberFormat('#,##0.00').format(card.currentDebt)}',
                              style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                        ]),
                        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                          const Text('Limit', style: TextStyle(color: Colors.white70, fontSize: 11)),
                          Text('₺${NumberFormat('#,##0.00').format(card.creditLimit)}',
                              style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                        ]),
                      ]),
                    ]),
                  ...
                  Padding(
                    ...
                    child: Column(children: [
                      LinearProgressIndicator(
                        ...
                        backgroundColor: Colors.grey.shade200,
                        color: usagePercent > 80 ? Colors.red : usagePercent > 50 ? Colors.orange : Colors.green,
                        minHeight: 8,
                        borderRadius: BorderRadius.circular(4),
                      ),
                      const SizedBox(height: 8),
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Text('Kullanım: %${usagePercent.toStringAsFixed(0)}',
                            style: const TextStyle(fontSize: 12, color: Colors.grey)),
                        Text('Müsait: ₺${NumberFormat('#,##0').format(card.availableLimit)}',
                            style: const TextStyle(fontSize: 12, color: Colors.grey)),
                      ]),
                      const SizedBox(height: 8),
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Text('Ekstre: Her ayın ${card.statementDay}. günü',
                            style: const TextStyle(fontSize: 11, color: Colors.grey)),
                        Text('Son ödeme: ${card.dueDay}. gün',
                            style: const TextStyle(fontSize: 11, color: Colors.grey)),
                      ]),
                      const SizedBox(height: 12),
                      if (card.currentDebt > 0) ...[
                        Container(
                          ...
                          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              const Text('Asgari Ödeme (%40)',
                                  style: TextStyle(fontSize: 11, color: Colors.orange)),
                              Text('₺${NumberFormat('#,##0.00').format(card.minimumPayment)}',
                            ...
                            Column(children: [
                              ElevatedButton(
                                ...
                                onPressed: () => _showPayCreditCard(card, isMinimum: true),
                                child: const Text('Asgari Öde', style: TextStyle(fontSize: 12)),
                              ),
                              const SizedBox(height: 4),
                              OutlinedButton(
                                ...
                                onPressed: () => _showPayCreditCard(card, isMinimum: false),
                                child: const Text('Manuel Öde', style: TextStyle(fontSize: 12)),
                              ),
                            ]),
                          ]),
                        ),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Text(isMinimum ? 'Asgari Ödeme' : 'Manuel Ödeme',
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (amount <= 0) return;
                final newDebt = (card.currentDebt - amount).clamp(0.0, double.infinity).toDouble();
              ...
              child: const Text('Ödemeyi Onayla', style: TextStyle(fontSize: 16)),
            ),
          ),
        ]),
      ),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('${card.bankName} - ${card.cardName}',
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const Text('Yeni Ekstre Tutarı', style: TextStyle(color: Colors.grey)),
          const SizedBox(height: 16),
          TextField(
            controller: amountC,
            keyboardType: TextInputType.number,
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: () async {
                ...
                final updated = card.copyWith(currentDebt: card.currentDebt + amount);
                await _db.updateCreditCard(updated);
                await _loadAll();
                if (context.mounted) Navigator.pop(context);
              },
              child: const Text('Ekstre Ekle', style: TextStyle(fontSize: 16)),
            ),
          ),
        ]),
      ),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => DraggableScrollableSheet(
        ...
        builder: (context, scrollController) => Padding(
          padding: const EdgeInsets.all(20),
          child: Column(children: [
            Text('${card.bankName} - ${card.cardName} Geçmişi',
                style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            if (statements.isEmpty)
              const Expanded(
                child: Center(child: Text('Henüz ekstre yok', style: TextStyle(color: Colors.grey))),
              )
            else
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            Text(DateFormat('MMMM yyyy').format(s.statementDate),
                                style: const TextStyle(fontWeight: FontWeight.bold)),
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                              decoration: BoxDecoration(
                                color: s.isPaid ? Colors.green.shade50 : Colors.orange.shade50,
                                borderRadius: BorderRadius.circular(8),
                              ),
                              child: Text(s.isPaid ? 'Ödendi' : 'Bekliyor',
                                  style: TextStyle(
                                      color: s.isPaid ? Colors.green : Colors.orange, fontSize: 12)),
                            ),
                          ]),
                          const SizedBox(height: 8),
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            Text('Ekstre: ₺${NumberFormat('#,##0.00').format(s.amount)}',
                                style: const TextStyle(fontSize: 13)),
                            Text('Ödenen: ₺${NumberFormat('#,##0.00').format(s.paidAmount)}',
                                style: const TextStyle(fontSize: 13, color: Colors.green)),
                          ]),
                          const SizedBox(height: 4),
                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            Text('Kalan: ₺${NumberFormat('#,##0.00').format(s.remainingAmount)}',
                                style: const TextStyle(color: Colors.red, fontSize: 13, fontWeight: FontWeight.bold)),
                            Text('Son Ödeme: ${DateFormat('dd MMM').format(s.dueDate)}',
                                style: const TextStyle(fontSize: 11, color: Colors.grey)),
                          ]),
                          const SizedBox(height: 8),
                          LinearProgressIndicator(
                            value: s.amount > 0 ? (s.paidAmount / s.amount).clamp(0.0, 1.0) : 0.0,
                            backgroundColor: Colors.grey.shade200,
                            color: Colors.green,
                            minHeight: 6,
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const Align(
                  ...
                  child: Text('Kart Rengi:', style: TextStyle(fontWeight: FontWeight.w600))),
              const SizedBox(height: 8),
              Row(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                      ...
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                  onPressed: () async {
                    if (bankNameC.text.isEmpty || cardNameC.text.isEmpty || limitC.text.isEmpty) return;
                    final card = CreditCard(
                      id: _uuid.v4(),
                      bankName: bankNameC.text,
                      cardName: cardNameC.text,
                      creditLimit: double.parse(limitC.text.replaceAll(',', '.')),
                      currentDebt: double.tryParse(debtC.text.replaceAll(',', '.')) ?? 0,
                  ...
                  child: const Text('Kartı Ekle', style: TextStyle(fontSize: 16)),
                ),
              ),
            ]),
          ),
  ...
  void _showAddTransaction() {
    ...
    final cats = ['Market', 'Faturalar', 'Kira', 'Ulaşım', 'Sağlık',
      'Eğlence', 'Giyim', 'Yemek', 'Eğitim', 'Diğer'];
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Row(children: [
                Expanded(
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: isExpense ? Colors.red : Colors.grey.shade200,
                        borderRadius: BorderRadius.circular(10),
                      ),
                      child: Center(child: Text('Gider',
                          style: TextStyle(color: isExpense ? Colors.white : Colors.black,
                              fontWeight: FontWeight.bold))),
                    ),
                  ),
                ),
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () => set(() { isExpense = false; selectedCard = null; }),
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 12),
                      decoration: BoxDecoration(
                        color: !isExpense ? Colors.green : Colors.grey.shade200,
                        borderRadius: BorderRadius.circular(10),
                      ),
                      child: Center(child: Text('Gelir',
                          style: TextStyle(color: !isExpense ? Colors.white : Colors.black,
                              fontWeight: FontWeight.bold))),
                    ),
                  ),
              ...
              if (isExpense && _creditCards.isNotEmpty) ...[
                ...
                DropdownButtonFormField<CreditCard?>(
                  ...
                  decoration: InputDecoration(
                      ...
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(10))),
                  items: [
                    const DropdownMenuItem(value: null, child: Text('Kartla ödeme yok')),
                    ..._creditCards.map((c) => DropdownMenuItem(
                          value: c,
              ...
              Row(children: [
                Checkbox(value: isFixed, onChanged: (v) => set(() => isFixed = v ?? false)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    final amount = double.parse(amountC.text.replaceAll(',', '.'));
                    final t = FinanceTransaction(
                      id: _uuid.v4(),
                      title: titleC.text,
                      amount: amount,
                        ...
                        if (spent > budget.limitAmount) {
                          ...
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text('⚠️ $cat bütçenizi ₺${asim.toStringAsFixed(2)} aştınız!'),
                                backgroundColor: Colors.orange,
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  final s = Subscription(
                    id: _uuid.v4(), title: titleC.text,
                    amount: double.parse(amountC.text),
                    category: 'Abonelik', billingDay: billingDay, color: '#6C63FF',
                  );
                  await _db.insertSubscription(s);
                  await NotificationService().scheduleSubscriptionReminder(
                    id: s.id.hashCode, subscriptionName: s.title,
                    dayOfMonth: billingDay, hour: 9, minute: 0,
                  );
                  await _loadAll();
                  if (context.mounted) Navigator.pop(context);
                },
  ...
  void _showAddDebt() {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Yeni Borç',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            TextField(controller: titleC,
                decoration: InputDecoration(labelText: 'Borç Adı (Kredi, Kart...)',
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))),
            const SizedBox(height: 12),
            TextField(controller: totalC, keyboardType: TextInputType.number,
                decoration: InputDecoration(labelText: 'Toplam Borç (₺)',
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))),
            const SizedBox(height: 12),
            TextField(controller: paidC, keyboardType: TextInputType.number,
                decoration: InputDecoration(labelText: 'Ödenen Miktar (₺)',
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))),
            const SizedBox(height: 12),
            TextField(controller: monthlyC, keyboardType: TextInputType.number,
                decoration: InputDecoration(labelText: 'Aylık Ödeme (₺)',
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))),
            const SizedBox(height: 12),
            TextField(controller: rateC, keyboardType: TextInputType.number,
                decoration: InputDecoration(labelText: 'Faiz Oranı % (opsiyonel)',
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  final d = Debt(
                    id: _uuid.v4(), title: titleC.text,
                    totalAmount: double.parse(totalC.text.replaceAll(',', '.')),
                    paidAmount: double.tryParse(paidC.text.replaceAll(',', '.')) ?? 0,
                    monthlyPayment: double.tryParse(monthlyC.text.replaceAll(',', '.')) ?? 0,
                    startDate: DateTime.now(),
                    interestRate: double.tryParse(rateC.text.replaceAll(',', '.')) ?? 0,
                  );
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, set) => Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 12),
            TextField(controller: limitC, keyboardType: TextInputType.number,
                decoration: InputDecoration(labelText: 'Aylık Limit (₺)',
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
