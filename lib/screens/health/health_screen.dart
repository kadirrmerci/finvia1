...
class _HealthScreenState extends State<HealthScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Sağlık & Alışkanlık',
    ...
    if (_records.isEmpty) {
      return Center(
        child: Column(
          ...
          const Text('Henüz kilo kaydı yok',
              style: TextStyle(color: Colors.grey, fontSize: 16)),
          const Text('+ butonuna basarak ekle',
        ...
        ..._records.map((r) => Dismissible(
              ...
              child: Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(
                      DateFormat('dd MMMM yyyy').format(r.date)),
        ...
        return Dismissible(
          ...
          onDismissed: (_) async {
            await NotificationService()
                .cancelNotification(4000 + habit.id.hashCode);
                        ...
                        GestureDetector(
                          onTap: () async {
                            ...
                            List<DateTime> newDays =
                                List.from(habit.completedDays);
                            if (isCompleted) {
                              newDays.removeWhere((d) =>
                                  d.year == today.year &&
                                  d.month == today.month &&
                                  d.day == today.day);
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, set) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text('Her gün hangi saatte kilo ölçüm hatırlatması gönderelim?'),
            const SizedBox(height: 16),
            GestureDetector(
              onTap: () async {
  ...
  void _showAddHabit() {
    ...
    final habitTemplates = [
      {'emoji': '🚭', 'title': 'Sigara Bırak', 'type': 'quit', 'motivation': 'Sağlıklı bir yaşam için'},
      {'emoji': '🏃', 'title': 'Spor Yap', 'type': 'build', 'motivation': 'Her gün daha güçlü'},
      {'emoji': '💧', 'title': 'Su İç', 'type': 'build', 'motivation': '8 bardak su içiyorum'},
      {'emoji': '📚', 'title': 'Kitap Oku', 'type': 'build', 'motivation': 'Her gün öğreniyorum'},
      {'emoji': '🧘', 'title': 'Meditasyon', 'type': 'build', 'motivation': 'Zihinsel huzur için'},
      {'emoji': '🍎', 'title': 'Sağlıklı Ye', 'type': 'build', 'motivation': 'Vücuduma iyi bakıyorum'},
    ];
