...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final Map<String, List<String>> _cityDistricts = {
    ...
    'Çorum': ['Alaca','Bayat','Boğazkale','Dodurga','İskilip','Kargı','Laçin','Mecitözü','Merkez','Oğuzlar','Ortaköy','Osmancık','Sungurlu','Uğurludağı'],
    ...
    'Erzurum': ['Aşkale','Aziziye','Çat','Hınıs','Horasan','İspir','Karayazı','Köprüköy','Merkez','Narman','Oltu','Olur','Palandöken','Pasinler','Pazaryolu','Şenkaya','Tekman','Tortum','Uzundere','Yakutiye'],
    ...
    'Yalova': ['Altınova','Armutlu','Çınarcık','Çiftlikkköy','Merkez','Termal'],
  ...
  Future<void> _submitEmail() async {
    if (_emailController.text.isEmpty || _passwordController.text.isEmpty) {
      _showError('Lütfen tüm alanları doldurun'); return;
    }
    if (!_isValidEmail(_emailController.text.trim())) {
      _showError('Geçerli bir email adresi girin'); return;
    }
    if (!_isLogin) {
      if (_nameController.text.isEmpty) { _showError('Ad soyad gerekli'); return; }
      if (_ageController.text.isEmpty) { _showError('Yaş gerekli'); return; }
      if (_selectedDistrict.isEmpty) { _showError('İlçe seçiniz'); return; }
    }
    ...
    try {
      if (_isLogin) {
        ...
        if (user != null) {
          ...
          try { await user.sendEmailVerification(); } catch (e) { debugPrint('Email gönderilemedi: $e'); }
      ...
      if (e.code == 'email-already-in-use') message = 'Bu email zaten kullanımda';
      if (e.code == 'weak-password') message = 'Şifre çok zayıf (min 6 karakter)';
      if (e.code == 'invalid-email') message = 'Geçersiz email adresi';
      if (e.code == 'invalid-credential') message = 'Email veya şifre hatalı';
      if (e.code == 'too-many-requests') message = 'Çok fazla deneme. Lütfen bekleyin';
      if (e.code == 'network-request-failed') message = 'İnternet bağlantısı yok';
      _showError(message);
    } catch (e) {
  ...
  Future<void> _sendOtp() async {
    if (_phoneController.text.isEmpty) { _showError('Telefon numarası gerekli'); return; }
    setState(() => _isLoading = true);
    try {
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        codeSent: (String verificationId, int? resendToken) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('SMS gönderildi!'), backgroundColor: Colors.green),
          );
        },
  ...
  Future<void> _verifyOtp() async {
    if (_otpController.text.isEmpty) { _showError('Doğrulama kodu gerekli'); return; }
    setState(() => _isLoading = true);
    try {
      ...
      if (userCred.user != null && _nameController.text.isNotEmpty) {
        ...
        await _saveUserToFirestore(userCred.user!, phone: '+90${_phoneController.text.trim()}');
      }
    } on FirebaseAuthException catch (e) {
      String message = 'Hata: ${e.code}';
      if (e.code == 'invalid-verification-code') message = 'Hatalı doğrulama kodu';
      if (e.code == 'session-expired') message = 'Kod süresi doldu, tekrar gönderin';
      _showError(message);
    } catch (e) {
      _showError('Beklenmedik hata: $e');
    } finally {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(
                        child: GestureDetector(
                          onTap: () => setState(() { _loginMethod = 'email'; _otpSent = false; }),
                          child: Container(
                            padding: const EdgeInsets.symmetric(vertical: 12),
                            decoration: BoxDecoration(
                              color: _loginMethod == 'email' ? const Color(0xFF89CC2A) : Colors.transparent,
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(color: const Color(0xFF89CC2A)),
                            ),
                            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                              Icon(Icons.email_outlined,
                                  color: _loginMethod == 'email' ? Colors.white : const Color(0xFF89CC2A),
                                  size: 18),
                      ...
                      Expanded(
                        child: GestureDetector(
                          onTap: () => setState(() { _loginMethod = 'phone'; _otpSent = false; }),
                          child: Container(
                            padding: const EdgeInsets.symmetric(vertical: 12),
                            decoration: BoxDecoration(
                              color: _loginMethod == 'phone' ? const Color(0xFF89CC2A) : Colors.transparent,
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(color: const Color(0xFF89CC2A)),
                            ),
                            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                              Icon(Icons.phone_outlined,
                                  color: _loginMethod == 'phone' ? Colors.white : const Color(0xFF89CC2A),
                                  size: 18),
                              const SizedBox(width: 6),
                              Text('Telefon',
                                  style: TextStyle(
                    ...
                    if (_loginMethod == 'email') ...[
                      ...
                      Center(
                        child: TextButton(
                          onPressed: () => setState(() { _isLogin = !_isLogin; _otpSent = false; }),
                          child: Text(
                      ...
                      if (!_otpSent) ...[
                        ...
                        Row(children: [
                          Container(
                            ...
                            child: const Text('+90', style: TextStyle(fontSize: 16)),
                          ),
                        ...
                        const Text('SMS ile gönderilen 6 haneli kodu girin',
